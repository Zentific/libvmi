...
void TreeDestHelper(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (x != nil) {
    ...
    if(tree->DestroyKey) tree->DestroyKey(x->key);
    if(tree->DestroyInfo) tree->DestroyInfo(x->info);
    free(x);
  }
}
...
void RBDelete(rb_red_blk_tree* tree, rb_red_blk_node* z){
  ...
  if (y != z) { /* y should not be nil in this case */
    ...
    if(tree->DestroyKey) tree->DestroyKey(z->key);
    if(tree->DestroyInfo) tree->DestroyInfo(z->info);
    y->left=z->left;
    y->right=z->right;
    y->parent=z->parent;
    y->red=z->red;
    ...
    if(tree->DestroyKey) tree->DestroyKey(y->key);
    if(tree->DestroyInfo) tree->DestroyInfo(y->info);
    if (!(y->red)) RBDeleteFixUp(tree,x);
    free(y);
  }
